    ...
    <main style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.white, fontFamily: FONT }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px 16px 48px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 10, color: COLORS.label, letterSpacing: "0.12em", marginBottom: 8 }}>STAKING</div>
        ...
        {/* Live stats */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 1, background: COLORS.border, marginBottom: 20 }}>
          {[
            ...
            <div key={label} style={{ background: COLORS.bg, padding: "14px 18px" }}>
              <div style={{ fontSize: 10, color: COLORS.label, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{label}</div>
        ...
        {/* Tier table */}
        <div style={{ border: `1px solid ${COLORS.border}`, marginBottom: 32 }}>
          <div style={{ padding: "12px 20px", borderBottom: `1px solid ${COLORS.border}`, fontSize: 10, color: COLORS.label, letterSpacing: "0.1em" }}>TIERS</div>
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr auto auto", gap: "0", fontSize: 12 }}>
            {/* header */}
            {["tier", "min $CUSTOS", "credit multiplier", ""].map((h, i) => (
              <div key={i} style={{ padding: "8px 20px", color: "#aaa", fontSize: 10, letterSpacing: "0.08em", borderBottom: `1px solid ${COLORS.borderDim}` }}>{h}</div>
            ))}
            {/* rows */}
            {[
              ["1", fmtThreshold(t1, TIER_AMOUNTS[1]), "1×", COLORS.dim],
              ["2", fmtThreshold(t2, TIER_AMOUNTS[2]), "2×", "#888"],
              ["3", fmtThreshold(t3, TIER_AMOUNTS[3]), "3×", COLORS.accent],
            ].map(([tier, amount, mult, col]) => (
              <div key={tier} style={{ display: "contents" }}>
                <div style={{ padding: "10px 20px", color: COLORS.white, borderBottom: `1px solid ${COLORS.surface}` }}>Tier {tier}</div>
                <div style={{ padding: "10px 20px", color: "#bbb", fontFamily: "ui-monospace, monospace", borderBottom: `1px solid ${COLORS.surface}` }}>{amount}</div>
                <div style={{ padding: "10px 20px", color: col as string, fontWeight: 700, borderBottom: `1px solid ${COLORS.surface}` }}>{mult}</div>
                <div style={{ borderBottom: `1px solid ${COLORS.surface}` }} />
              </div>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 36 }}>
          <div style={{ fontSize: 10, color: COLORS.label, letterSpacing: "0.1em", marginBottom: 16 }}>HOW TO STAKE (CLI)</div>

          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12, color: "#bbb", marginBottom: 8 }}>1. approve $CUSTOS to MineController</div>
            <CodeBlock>{`cast send ${CONTRACTS.CUSTOS_TOKEN} \\
...
cast send ${CONTRACTS.MINE_CONTROLLER} \\
  ...
  --rpc-url https://mainnet.base.org --private-key $PRIVATE_KEY`}</CodeBlock>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <CodeBlock>{`cast call ${CONTRACTS.MINE_CONTROLLER} \\
  "getStake(address)((uint256,bool,uint256,uint256))" \\
  $YOUR_WALLET \\
  --rpc-url https://mainnet.base.org
# returns: (amount, withdrawalQueued, unstakeEpochId, stakedIndex)`}</CodeBlock>
          </div>

          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12, color: "#bbb", marginBottom: 8 }}>4. queue unstake (tokens return at epoch close)</div>
            <CodeBlock>{`cast send ${CONTRACTS.MINE_CONTROLLER} \\
  "unstake()" \\
  --rpc-url https://mainnet.base.org --private-key $PRIVATE_KEY
