  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "ui-monospace, 'Cascadia Code', 'Fira Code', monospace" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "36px 24px 60px" }}>
        ...
        {epochIds.length === 0 ? (
          <div style={{ border: "1px solid #1a1a1a", padding: 48, textAlign: "center", color: "#555" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                        ...
                        {expired && (
                          <span style={{ fontSize: 10, color: "#555", border: "1px solid #1a1a1a", padding: "2px 8px" }}>expired</span>
                        )}
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "6px 32px", fontSize: 12 }}>
                        <div>
                          <div style={{ color: "#555", marginBottom: 2, fontSize: 10 }}>REWARD POOL</div>
                          <div style={{ color: "#dc2626", fontWeight: 700 }}>{epoch ? formatCustos(epoch.rewardPool) : "—"}</div>
                          <div style={{ color: "#444", fontSize: 10 }}>$CUSTOS</div>
                        </div>
                        <div>
                          <div style={{ color: "#555", marginBottom: 2, fontSize: 10 }}>TOTAL CREDITS</div>
                          <div>{epoch?.totalCredits?.toString() ?? "—"}</div>
                          <div style={{ color: "#444", fontSize: 10 }}>correct reveals</div>
                        </div>
                        <div>
                          <div style={{ color: "#555", marginBottom: 2, fontSize: 10 }}>CLAIM DEADLINE</div>
                          <div style={{ fontSize: 11 }}>
                            {epoch?.claimDeadline ? new Date(Number(epoch.claimDeadline) * 1000).toLocaleDateString() : "—"}
                          </div>
                          <div style={{ color: "#444", fontSize: 10 }}>30 days after close</div>
                        </div>
                      </div>
                    </div>
                    ...
                    {settled && !expired && (
                      <div style={{ textAlign: "right", marginLeft: 24, flexShrink: 0 }}>
                        <div style={{ fontSize: 10, color: "#555", marginBottom: 6 }}>TO CLAIM</div>
                        <code style={{ fontSize: 11, color: "#888", display: "block", lineHeight: 1.5 }}>
        ...
        <div style={{ marginTop: 32, border: "1px solid #1a1a1a", padding: "16px 20px" }}>
          <div style={{ fontSize: 10, color: "#555", letterSpacing: "0.1em", marginBottom: 12 }}>CLAIM VIA CLI</div>
